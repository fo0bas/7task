...
document.getElementById('copy-button').addEventListener('click', () => {
    const taskText = document.getElementById('full-task-text').textContent; // Получаем текст задачи
    navigator.clipboard.writeText(taskText) // Копируем текст в буфер обмена
        
        .catch(err => {
            console.error('Ошибка при копировании текста: ', err); // Обработка ошибок
        });
});
...
//БЛОК СКАЧИВАНИЯ ЗАДАЧ.
document.getElementById('upload-button').addEventListener('click', () => {
    document.getElementById('upload-input').click(); // Открыть диалог выбора файла
});
